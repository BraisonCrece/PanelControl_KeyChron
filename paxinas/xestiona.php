...
<head>
    <meta charset="UTF-8">
    <title>Keychron | Principal</title>
...
<body>
    ...
    if (isset($_POST['opt'])) {
        ...
        } else if ($opt == "alugados") {
            $query = "SELECT * FROM artigo WHERE ID_artigo IN (SELECT ID_artigo FROM aluga WHERE devolto = 0)";
        }
    ...
    <div class="container" id="centro">
        <div class="d-flex card p-2 opcions bg-dark shadow">
            <form action="xestiona.php" method="post" class="d-flex gap-2 justify-content-center">
                <button name="opt" value="todos" class="btn btn-secondary">Mostrar todos os artigos</button>
                <button name="opt" value="alugados" class="btn btn-secondary">Mostrar artigos alugados</button>
            </form>
        </div>
        <div class="card p-3 text-center shadow">
            <form action="./artigo/detalles.php" method="post">
                <div class="d-flex gap-2 galeria justify-content-center">
                    <?php
                    require("../funcions/funcions.php");
                    if (isset($_POST['opt'])) {
                        mostrar_artigos($conexion, $query);
                    } else if (isset($_POST["buscar"])) {
                        print_buscar($conexion, $buscar);
                    } else {
                        echo "<h1 style='font-size:x-large; font-style:italic;'>Selecciona unha opción</h1>";
                    }
                    ?>
                </div>
            </form>
